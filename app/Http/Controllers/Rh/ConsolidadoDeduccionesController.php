...
class ConsolidadoDeduccionesController extends Controller
{
    public function index(Request $request, CalcularConsolidadoDeduccionesService $calcularService): Response
    {
        ...
        $fechaInicioGlobal = $config->periodo_actual_inicio
            ? Carbon::parse($config->periodo_actual_inicio)
            : now()->startOfMonth();
        $fechaFinGlobal = $config->periodo_actual_fin
            ? Carbon::parse($config->periodo_actual_fin)
            : now()->endOfMonth();
