...
class OptimizarImagenActivoService
{
    private const MAX_BYTES = 5 * 1024 * 1024;
    private const MAX_SIDE = 1600;
    private const WEBP_QUALITY = 82;

    public function ejecutar(UploadedFile $file, int $activoId): array
    {
        if ($file->getSize() > self::MAX_BYTES) {
            throw ValidationException::withMessages([
                'fotos' => 'Cada imagen debe pesar menos de 5 MB.',
            ]);
        }
        ...
        if (!$loaded) {
            throw ValidationException::withMessages([
                'fotos' => 'Formato de imagen no soportado. Use JPG, PNG o WEBP.',
            ]);
        }
        ...
        if (!function_exists('imagewebp')) {
            imagedestroy($resource);
            throw ValidationException::withMessages([
                'fotos' => 'WebP no está disponible en el servidor.',
            ]);
        }
