...
class ImportarDatosFiscalesService
{
    ...
    public function extraer(UploadedFile $archivo): array
    {
        ...
        if (!$path || !is_readable($path)) {
            throw ValidationException::withMessages([
                'archivo_fiscal' => 'No se pudo leer el archivo subido.',
            ]);
        }
    ...
    public function datosFiscalesDesdeCliente(?\App\Models\Cliente $cliente): ?array
    {
        if (!$cliente) {
            return null;
        }
    ...
    private function obtenerFilasDesdeCsv(string $path): array
    {
        $file = fopen($path, 'r');
        if ($file === false) {
            throw ValidationException::withMessages(['archivo_fiscal' => 'No se pudo leer el archivo CSV.']);
        }

        $headersRaw = fgetcsv($file);
        if ($headersRaw === false) {
            fclose($file);
            throw ValidationException::withMessages(['archivo_fiscal' => 'El archivo CSV está vacío.']);
        }
    ...
    {
        if (!is_readable($path)) {
            throw ValidationException::withMessages(['archivo_fiscal' => 'No se pudo leer el archivo Excel.']);
        }
        ...
        try {
            foreach ((new FastExcel)->import($path) as $row) {
                ...
                if ($filasEscaneadas > 2) {
                    throw ValidationException::withMessages([
                        'archivo_fiscal' => 'El archivo solo puede contener una fila de datos fiscales.',
                    ]);
                }

                $mapeada = $this->mapearFilaAsociativa($rowArray);
                if ($mapeada === null || $this->filaVacia(array_values($mapeada))) {
                    continue;
                }
                $filas[] = $mapeada;
            }
            ...
            throw ValidationException::withMessages([
                'archivo_fiscal' => 'No se pudo procesar el archivo Excel. Descargue la plantilla oficial.',
            ]);
        }

        if (!$cabecerasValidadas) {
            throw ValidationException::withMessages(['archivo_fiscal' => 'El archivo Excel está vacío.']);
        }
    ...
    private function mapearFilaAsociativa(array $row): ?array
    {
        ...
        if ($faltantes !== []) {
            throw ValidationException::withMessages([
                'archivo_fiscal' => 'Faltan columnas obligatorias: ' . implode(', ', $faltantes) . '.',
            ]);
        }
    ...
    private function mapearCabeceras(array $headersRaw): array
    {
        ...
        foreach ($headersRaw as $index => $header) {
            $mapa[$this->normalizarCabecera((string) $header)] = $index;
        }
    ...
    private function extraerFila(array $row, array $mapa): array
    {
        ...
        $datos = [];
        foreach (self::COLUMNAS_REQUERIDAS as $columna => $clave) {
            $datos[$clave] = trim((string) ($row[$mapa[$columna]] ?? ''));
        }
    ...
    {
        if (count($filas) === 0) {
            throw ValidationException::withMessages(['archivo_fiscal' => 'El archivo debe contener exactamente una fila de datos fiscales.']);
        }

        if (count($filas) > 1) {
            throw ValidationException::withMessages(['archivo_fiscal' => 'El archivo solo puede contener una fila de datos fiscales.']);
        }
        ...
        foreach (self::COLUMNAS_REQUERIDAS as $columna => $clave) {
            if ($fila[$clave] === '') {
                throw ValidationException::withMessages(['archivo_fiscal' => "La columna {$columna} es obligatoria."]);
            }
        }

        $rfc = strtoupper(preg_replace('/\s+/', '', $fila['rfc']));
        if (!preg_match('/^[A-ZÑ&]{3,4}\d{6}[A-Z0-9]{3}$/u', $rfc)) {
            throw ValidationException::withMessages(['archivo_fiscal' => 'El RFC no tiene un formato válido.']);
        }

        if (!preg_match('/^\d{5}$/', $fila['codigo_postal'])) {
            throw ValidationException::withMessages(['archivo_fiscal' => 'El código postal debe tener 5 dígitos.']);
        }
