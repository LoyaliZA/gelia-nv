...
{
    /**
     * Procesa el CSV de WooCommerce e inserta los productos en la base de datos local.
     * * Por qué: Aísla la lógica de lectura de archivos y manipulación de DB del controlador.
     * Cómo: Recibe la ruta física del archivo, mapea SKUs y realiza inserciones por lotes.
     */
    ...
    /**
     * Genera un mapa de SKU a ID para resolver jerarquías (Variaciones).
     */
    ...
    /**
     * Inserta los productos en la base de datos en fragmentos (chunks).
     */
    private function procesarEInsertarProductos(string $path, array $skuToIdMap): void
    {
        ...
        $fileIn = fopen($path, 'r');
        $headersRaw = fgetcsv($fileIn); // Saltar cabeceras
        $headers = array_map(fn($i) => strtolower(trim((string)$i)), $headersRaw);
        ...
        // Inserción por lotes para optimizar memoria
