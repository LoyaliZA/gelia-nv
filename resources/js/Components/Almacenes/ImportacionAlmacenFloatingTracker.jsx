...
import { X, StopCircle, Play, FileSpreadsheet } from 'lucide-react';
import ImportacionResumenModal from '@/Components/Almacenes/ImportacionResumenModal';
import {
    clearImportacionAlmacenTracking,
    dismissImportacionAlmacenTracking,
    ESTADOS_ACTIVOS,
    ESTADOS_REANUDABLES,
    etiquetaTipoImportacion,
    getStoredImportacionAlmacenLogId,
    IMPORTACION_ALMACEN_DISMISSED_EVENT,
    IMPORTACION_ALMACEN_STARTED_EVENT,
    IMPORTACION_ALMACEN_STORAGE_KEY,
} from '@/utils/importacionAlmacenTracker';
...
function logEsVisible(log) {
    ...
    if (ESTADOS_REANUDABLES.includes(log.estado) && log.procesados < log.total_filas) return true;
    if (log.estado === 'en_proceso' && log.procesados < log.total_filas) return true;
    return false;
}

function logPuedeContinuar(log) {
    if (!log || log.procesados >= log.total_filas) return false;
...
export default function ImportacionAlmacenFloatingTracker({ canView = false, canManage = false }) {
    const { props } = usePage();
    const importacionActivaInertia = props.importacion_almacen_activa;
    ...
    const fetchLog = useCallback(async (id) => {
        const res = await fetch(route('almacenes.importaciones.progreso', id), {
            headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
        });
        if (res.status === 404) return null;
        if (!res.ok) throw new Error('No se pudo consultar el progreso de la importación.');
        return res.json();
    }, []);
    ...
    const conectarLog = useCallback((id, dataInicial = null) => {
        ...
        if (typeof window !== 'undefined') {
            localStorage.setItem(IMPORTACION_ALMACEN_STORAGE_KEY, String(id));
        }
    ...
    const finalizarSeguimiento = useCallback((data, motivo) => {
        ...
        if (motivo === 'completado') {
            detenerPolling();
            clearImportacionAlmacenTracking();
            emitToast('Importación completada.', 'success');
            if (data?.resumen) {
        ...
        if (motivo === 'error' || motivo === 'interrumpido') {
            ...
            if (toastNotificadoRef.current !== data?.id) {
                ...
                emitToast(data?.mensaje_error || 'La importación falló.', 'error');
            }
            return;
        }
        ...
        if (motivo === 'cancelado') {
            emitToast('Importación cancelada.', 'success');
            setLogId(null);
            setLog(null);
        }
    }, [detenerPolling]);
    ...
    const descubrirActivo = useCallback(async () => {
        try {
            const res = await fetch(route('almacenes.importaciones.activo'), {
                headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
            });
            if (!res.ok) return;
    ...
    useEffect(() => {
        ...
        const stored = getStoredImportacionAlmacenLogId();
        if (stored) {
            conectarLog(stored);
            return;
        }

        if (importacionActivaInertia?.id && logEsVisible(importacionActivaInertia)) {
            conectarLog(importacionActivaInertia.id, importacionActivaInertia);
            return;
        }

        descubrirActivo();
    }, [canView, importacionActivaInertia, conectarLog, descubrirActivo]);
    ...
    useEffect(() => {
        ...
        window.addEventListener(IMPORTACION_ALMACEN_STARTED_EVENT, onStarted);
        window.addEventListener(IMPORTACION_ALMACEN_DISMISSED_EVENT, onDismissed);

        return () => {
            window.removeEventListener(IMPORTACION_ALMACEN_STARTED_EVENT, onStarted);
            window.removeEventListener(IMPORTACION_ALMACEN_DISMISSED_EVENT, onDismissed);
        };
        ...
        const poll = async () => {
            try {
                ...
                if (data.estado === 'completado') {
                    finalizarSeguimiento(data, 'completado');
                } else if (data.estado === 'cancelado') {
                    finalizarSeguimiento(data, 'cancelado');
    ...
    const cancelar = async () => {
        if (!logId || !canManage) return;
        if (!window.confirm('¿Cancelar esta importación?')) return;

        setAccionando(true);
        try {
            const res = await fetch(route('almacenes.importaciones.cancelar', logId), {
                method: 'DELETE',
                headers: {
    ...
    const continuar = async () => {
        ...
        setAccionando(true);
        try {
            const res = await fetch(route('almacenes.importaciones.continuar', logId), {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': csrfToken(),
                    Accept: 'application/json',
    ...
    if (resumen) {
        return <ImportacionResumenModal data={resumen} onClose={() => setResumen(null)} />;
    }

    if (!canView || !logId) return null;

    const pct = log
        ? Math.round((log.procesados / Math.max(log.total_filas, 1)) * 100)
        : 0;
    const activo = log && ESTADOS_ACTIVOS.includes(log.estado);
    const puedeContinuar = logPuedeContinuar(log);
    const titulo = log ? (log.etiqueta_tipo || etiquetaTipoImportacion(log.tipo)) : 'Importación';
    ...
    if (minimized) {
        return (
            <button
                ...
                title="Ver progreso de importación"
            >
            ...
            <div className="relative p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3 min-w-0">
                        <FileSpreadsheet className="w-10 h-10 shrink-0" style={{ color: 'var(--color-primario)' }} />
                ...
                <div>
                    ...
                    <div className="flex justify-between text-[10px] font-black uppercase theme-text-muted">
                        <span>{log?.procesados ?? 0} / {log?.total_filas ?? '—'}</span>
