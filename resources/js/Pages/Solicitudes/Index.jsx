...
/* --- SECCIÓN: ESTILOS GLOBALES ESPECÍFICOS --- */
// NOTA: Se eliminaron las clases .theme-surface y .theme-element porque ahora el AppLayout 
// se encarga de inyectarlas globalmente para soportar el Efecto Cristal.
const ESTILOS_GLOBALES = `
    .theme-hover-bg { transition: background-color 0.3s; }
    .theme-hover-bg:hover { background-color: rgba(0, 0, 0, 0.03); }
    .dark .theme-hover-bg:hover { background-color: rgba(255, 255, 255, 0.03); }
        ...
        <AppLayout auth={auth}>
            ...
            {modalAbierto && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center theme-overlay p-4 overflow-y-auto">
                    {/* .theme-surface respetará el glassmorphism global */}
                    <div className="theme-surface border-2 theme-border rounded-[3rem] p-8 md:p-12 shadow-2xl max-w-5xl w-full relative tab-content my-auto">
                        <button onClick={() => setModalAbierto(false)} className="absolute top-6 right-6 p-3 theme-element rounded-2xl transition-all theme-hover-bg" style={{ color: 'var(--color-primario)' }}>
                            <X className="w-5 h-5" />
                        </button>
                        ...
                        <form onSubmit={guardarSolicitud} className="grid grid-cols-1 lg:grid-cols-5 gap-12">
                            <div className="lg:col-span-3 space-y-8">
                                ...
                                <div className="space-y-3 relative">
                                    ...
                                    <div className="relative">
                                        {/* Z-INDEX y POINTER EVENTS NONE APLICADOS AQUÍ */}
                                        ...
                                        <input 
                                            ...
                                            // PADDING SIMÉTRICO pl-12 y pr-4 
                                            className="w-full pl-11 pr-4 py-4 theme-element border-2 theme-border rounded-2xl theme-text-main font-bold outline-none transition-all focus:border-[var(--color-primario)] focus:ring-1 focus:ring-[var(--color-primario)]" 
                                            autoComplete="off"
                                        />
                                    </div>
                                    ...
                                    {mostrarDropdown && (
                                        <div className="absolute top-[85px] left-0 right-0 theme-surface border-2 theme-border rounded-2xl shadow-2xl z-50 max-h-64 overflow-y-auto custom-scrollbar">
                                            <div className="sticky top-0 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md px-4 py-2 border-b theme-border flex justify-between items-center z-10">
                                                ...
                                                <button type="button" onClick={() => setMostrarDropdown(false)} className="text-[10px] font-black uppercase transition-transform hover:scale-105" style={{ color: 'var(--color-primario)' }}>Cerrar</button>
                                            </div>
                                            {buscandoCliente ? (
                                                <div className="p-6 text-center text-xs font-bold theme-text-muted animate-pulse italic">Consultando matriz de datos...</div>
                                            ) : listaClientes.map(c => (
                                                <div key={c.id} onClick={() => seleccionarCliente(c)} className="p-4 border-b theme-border last:border-b-0 theme-hover-bg cursor-pointer transition-colors flex justify-between items-center group">
                                                    <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-3">
                                        ...
                                        <input 
                                            ...
                                            required 
                                            className="w-full px-4 py-4 theme-element border-2 theme-border rounded-2xl theme-text-main font-bold outline-none transition-all focus:border-[var(--color-primario)] focus:ring-1 focus:ring-[var(--color-primario)]" 
                                        />
                                    </div>
                                    <div className="space-y-3">
                                        ...
                                        <select 
                                            ...
                                            required 
                                            className="w-full px-4 py-4 theme-element border-2 theme-border rounded-2xl theme-text-main font-bold outline-none cursor-pointer appearance-none transition-all focus:border-[var(--color-primario)] focus:ring-1 focus:ring-[var(--color-primario)]"
                                        >
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                <div className="space-y-3">
                                    ...
                                    <label className="flex flex-col items-center justify-center h-48 border-2 border-dashed theme-border rounded-3xl cursor-pointer theme-element theme-hover-bg transition-all" style={{ borderColor: data.evidencia ? 'var(--color-primario)' : '' }}>
                                        <Upload className="w-8 h-8 mb-2 theme-text-muted" style={{ color: data.evidencia ? 'var(--color-primario)' : '' }} />
                                        <p className="text-[10px] theme-text-muted uppercase font-black px-4 text-center truncate w-full">{data.evidencia ? data.evidencia.name : 'Vincular archivo_'}</p>
            ...
            {/* VISTA PRINCIPAL */}
            <div className="w-full p-4 md:p-6 lg:p-12 space-y-8 md:space-y-12">
                
                {/* ENCABEZADO CORREGIDO: AHORA ENVUELTO EN SU PROPIA TARJETA CON EFECTO CRISTAL */}
                <header className="tab-content theme-surface border-2 theme-border rounded-[2rem] md:rounded-[3rem] p-6 md:p-8 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                    ...
                    {can('crear_solicitud') && (
                        <button onClick={() => { setModalAbierto(true); }} className="flex items-center justify-center gap-2 px-8 py-4 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl hover:scale-105 transition-all w-full md:w-auto" style={{ backgroundColor: 'var(--color-primario)' }}>
                            <Plus className="w-4 h-4" /> Nueva Solicitud
                        </button>
                    )}
                </header>

                <div className="tab-content flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
                    <div className="flex gap-2 p-1.5 theme-surface border theme-border rounded-2xl w-full lg:w-fit overflow-x-auto custom-scrollbar">
                    ...
                    <div className="relative w-full lg:w-80 shrink-0">
                        {/* ÍCONO ARREGLADO CON z-10 y pointer-events-none */}
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 theme-text-muted z-10 pointer-events-none" />
                        <input 
                            type="text" 
                            ...
                            className="w-full pl-11 pr-4 py-3 theme-element border theme-border rounded-xl text-xs font-bold theme-text-main outline-none focus:border-[var(--color-primario)] focus:ring-1 focus:ring-[var(--color-primario)] transition-all" 
                        />
                    </div>
                </div>

                <div className="tab-content theme-surface border-2 theme-border rounded-[2rem] md:rounded-[3rem] p-4 md:p-8 lg:p-12 shadow-sm overflow-hidden w-full">
                    {/* Contenedor que permite scroll horizontal seguro en pantallas pequeñas */}
                                    ...
                                    return (
                                        <tr key={solicitud.id} className="border-b theme-border theme-hover-bg transition-colors">
                                            <td className="py-5 pl-4">
                                                ...
                                                <div className="text-[9px] font-bold theme-text-muted mt-1 uppercase truncate max-w-[120px]">{solicitud.vendedor?.name}</div>
                                            </td>
                                            <td className="py-5">
                                            ...
                                            <td className="py-5">
                                                <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-transparent ${estatus.clase} whitespace-nowrap`}>
                                            ...
                                            <td className="py-5 text-center relative">
                                                <button onClick={() => setMenuAbierto(menuAbierto === solicitud.id ? null : solicitud.id)} className="p-2 theme-hover-bg rounded-xl transition-colors inline-flex border border-transparent hover:border-zinc-300 dark:hover:border-zinc-700 outline-none">
                                                ...
                                                {menuAbierto === solicitud.id && (
                                                    <>
                                                        ...
                                                        <div className="absolute right-10 top-0 theme-surface border-2 theme-border shadow-2xl rounded-2xl p-2 z-20 w-56 flex flex-col gap-1 text-left animate-fade-in">
                                                            {can('confirmar_pago') && !solicitud.pago_confirmado && <button onClick={() => router.put(route('solicitudes.confirmar_pago', solicitud.id))} className="flex items-center gap-3 px-3 py-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg text-[10px] font-bold uppercase outline-none"><CreditCard className="w-3.5 h-3.5" /> Confirmar Pago</button>}
                                                            {can('verificar_auxiliar') && <button onClick={() => iniciarRespuesta(solicitud, 3)} className="flex items-center gap-3 px-3 py-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-lg text-[10px] font-bold uppercase outline-none"><CheckSquare className="w-3.5 h-3.5" /> Marcar Verificado</button>}
                                                            {can('ejecutar_tags') && (
                                                                <>
                                                                    <button onClick={() => iniciarRespuesta(solicitud, 2)} className="flex items-center gap-3 px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg text-[10px] font-bold uppercase outline-none" style={{ color: 'var(--color-primario)' }}><CheckCircle2 className="w-3.5 h-3.5" /> Aprobar Proceso</button>
                                                                    <button onClick={() => iniciarRespuesta(solicitud, 4)} className="flex items-center gap-3 px-3 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-[10px] font-bold uppercase outline-none"><AlertOctagon className="w-3.5 h-3.5" /> Reportar Error</button>
                                                                </>
                                                            )}
                                                            {can('ver_auditoria') && <button className="flex items-center gap-3 px-3 py-2 hover:bg-purple-50 dark:hover:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-lg text-[10px] font-bold uppercase outline-none"><History className="w-3.5 h-3.5" /> Ver Bitácora</button>}
                                                        </div>
                                                    </>
                                                )}
                                            </td>
