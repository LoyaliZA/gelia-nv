...
import { X, Edit2, Info, AlertOctagon, Upload } from 'lucide-react';
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-black/60 backdrop-blur-md animate-fade-in" onClick={onClose}>
            <div onPaste={handlePasteRespuesta} className="w-full max-w-3xl theme-surface border theme-border shadow-2xl rounded-[2.5rem] p-10 flex flex-col relative modal-pop" onClick={e => e.stopPropagation()}>
                <button onClick={onClose} className="absolute top-6 right-6 p-3 theme-text-muted hover:theme-text-main theme-element border theme-border rounded-2xl outline-none hover:scale-110"><X className="w-5 h-5" /></button>
                ...
                <form onSubmit={enviarRespuesta} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-8">
                        <div className="p-5 rounded-2xl border flex items-start gap-4" style={{ backgroundColor: esReporteError ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)', borderColor: esReporteError ? 'rgba(239, 68, 68, 0.3)' : 'rgba(52, 211, 153, 0.3)' }}>
                            {esReporteError ? <AlertOctagon className="w-6 h-6 text-red-500 mt-0.5" /> : <Info className="w-6 h-6 text-emerald-500 mt-0.5" />}
                            <div><p className="text-sm font-black uppercase" style={{ color: esReporteError ? '#ef4444' : '#10b981' }}>{esReporteError ? 'Reporte de Error' : 'Aprobación'}</p></div>
                        </div>
                        <div className="space-y-2"><label className="text-[10px] font-black uppercase theme-text-muted tracking-widest ml-1">Observaciones / Motivo</label><textarea required={esReporteError} rows="6" value={data.motivo} onChange={e => setData('motivo', e.target.value)} className="w-full p-5 theme-surface border theme-border rounded-xl theme-text-main text-sm font-bold outline-none focus:ring-2 shadow-sm resize-none"></textarea></div>
                    </div>
                    <div className="space-y-2 flex flex-col">
                        ...
                        <button type="submit" disabled={processing} className="w-full py-5 text-white rounded-2xl font-black uppercase tracking-widest text-[12px] hover:scale-105 transition-all mt-4" style={{ backgroundColor: esReporteError ? '#ef4444' : 'var(--color-primario)' }}>{processing ? 'Registrando...' : 'Confirmar Acción'}</button>
                    </div>
                </form>
            </div>
        </div>, document.body
