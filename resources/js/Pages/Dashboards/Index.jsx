...
export default function AdminDashboard({ auth, estadisticas = {} }) {
    ...
    if (can('gestionar_usuarios')) {
        accesosDisponibles.push({ titulo: 'Control de Usuarios', desc: 'Administrar roles y personal.', icon: Users, href: route('admin.usuarios'), color: 'text-pink-500', bgColor: 'bg-pink-500/10', hoverBg: 'hover:bg-pink-500', borderColor: 'border-pink-500/20' });
        accesosDisponibles.push({ titulo: 'Generar Accesos', desc: 'Crear enlaces seguros.', icon: UserPlus, href: route('admin.enlaces'), color: 'theme-text-main', bgColor: 'theme-element', hoverBg: 'hover:bg-[#1A1A1A] dark:hover:bg-white', borderColor: 'theme-border' });
    }
            ...
            <div className="max-w-[1400px] mx-auto p-6 md:p-12 space-y-10 min-h-screen">
                ...
                <header className="fade-in space-y-4">
                    <div className="flex items-center space-x-3">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-pink-600">Protocolo de Mando Global</p>
                    </div>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <h1 className="text-4xl md:text-5xl font-black italic tracking-tighter uppercase theme-text-main leading-tight">
                            BIENVENIDO, <span className="text-pink-500">{auth?.user?.name ? auth.user.name.split(' ')[0] : 'USUARIO'}</span>
                        </h1>
                        <div className="flex items-center gap-4 p-4 theme-surface border-2 theme-border rounded-2xl shadow-sm">
                ...
                {can('ver_auditoria') && (
                    <div className="fade-in grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div className="theme-surface border-2 theme-border p-8 rounded-3xl shadow-sm flex flex-col justify-center">
                        ...
                        <div className="theme-surface border-2 theme-border p-8 rounded-3xl shadow-sm flex flex-col justify-center">
                            ...
                            <h3 className="text-4xl font-black italic text-pink-500">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                        ...
                        {accesosDisponibles.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {accesosDisponibles.map((item, idx) => (
                                    <Link key={idx} href={item.href} className="fade-in theme-surface border-2 theme-border p-8 rounded-[2.5rem] shadow-sm hover:border-pink-500 transition-all group relative overflow-hidden flex flex-col">
                    ...
                    {can('ver_auditoria') && (
                        <div className="space-y-6 fade-in">
                            ...
                            <div className="theme-surface border-2 theme-border rounded-[2.5rem] p-6 shadow-sm">
                                <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-zinc-200 dark:before:via-[#333] before:to-transparent">
                                    {actividadReciente.map((log) => (
                                        <div key={log.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                            <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 theme-surface ${log.bg} shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-sm z-10`}>
                                                <log.icon className={`w-4 h-4 ${log.color}`} />
                                            </div>
                                            <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] theme-element border-2 theme-border p-4 rounded-2xl shadow-sm">
