...
export default function TablaOrigenesPedido({ datos = [] }) {
    ...
    const abrirEditar = (item) => {
        ...
        setData({
            nombre: item.nombre,
            requiere_logistica: item.requiere_logistica,
            activo: item.activo,
        });
        setModalAbierto(true);
    };
    ...
    const handleSubmit = (e) => {
        ...
        const ruta = itemActual
            ? route('admin.catalogos.origenes_pedido.update', itemActual.id)
            : route('admin.catalogos.origenes_pedido.store');
        accion(ruta, { onSuccess: () => { setModalAbierto(false); reset(); } });
    };

    const confirmDelete = () => {
        router.delete(route('admin.catalogos.origenes_pedido.destroy', itemActual.id), {
    ...
    return (
        <div>
            <GeliaLoader isVisible={processing} message="Guardando Origen_" />
            <div className="p-6 md:p-8 border-b theme-border flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-black italic theme-text-main uppercase tracking-tighter m-0">Orígenes de Pedido_</h2>
                ...
                <table className="w-full border-collapse">
                    <thead>
                        <tr className="border-b-2 border-[var(--color-primario)]/30">
                            ...
                            <th className="px-6 py-4 text-left text-[9px] font-black theme-text-muted uppercase tracking-widest">Logística_</th>
                    ...
                    <tbody>
                        {datos.map((item) => (
                            <tr key={item.id} className="border-b theme-border last:border-0">
                                ...
                                <td className="px-6 py-5 text-xs font-bold theme-text-muted">{item.requiere_logistica ? 'Requiere envío' : 'Mostrador / piso'}</td>
                    ...
                    <div className="w-full max-w-md theme-surface border theme-border rounded-[2rem] p-8 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <label className="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" checked={data.requiere_logistica} onChange={(e) => setData('requiere_logistica', e.target.checked)} className="w-4 h-4" />
                                <span className="text-sm font-bold theme-text-main">Requiere logística (paquetería / envío)</span>
                            </label>
                            <label className="flex items-center gap-3 cursor-pointer">
