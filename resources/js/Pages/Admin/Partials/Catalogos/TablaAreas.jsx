...
import { MapPin, Edit2, Trash2, Plus, X, Save, AlertTriangle, Building2 } from 'lucide-react';
import GeliaLoader from '../../../../Components/GeliaLoader';

export default function TablaAreas({ datos = [], departamentos = [] }) {
    ...
    const abrirEditar = (item) => {
        setItemActual(item);
        setData({ nombre: item.nombre, departamento_id: item.departamento_id });
        setModalAbierto(true);
    };
    ...
    const handleSubmit = (e) => {
        ...
        const ruta = itemActual ? route('admin.catalogos.areas.update', itemActual.id) : route('admin.catalogos.areas.store');
        
        accion(ruta, { onSuccess: () => { setModalAbierto(false); reset(); } });
    };

    const confirmDelete = () => {
        post(route('admin.catalogos.areas.destroy', itemActual.id), {
            _method: 'delete',
    ...
    return (
        <div>
            <GeliaLoader isVisible={processing} message="Guardando Área_" />
            ...
            <div className="p-6 md:p-8 border-b theme-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black italic theme-text-main uppercase tracking-tighter m-0">Áreas Operativas_</h2>
                            ...
                            <tr key={item.id} className="group transition-all duration-150 border-b theme-border last:border-0 hover:ring-2 hover:ring-inset hover:ring-[var(--color-primario)]/30">
                                ...
                                <td className="px-6 py-5">
                                    <span className="px-3 py-1.5 rounded-xl theme-element border theme-border text-[10px] font-black uppercase tracking-widest theme-text-main shadow-sm">
                                        {item.departamento?.nombre || 'Desconocido'}
                                    </span>
                                </td>
                    ...
                    <div className="w-full max-w-lg theme-surface theme-border border shadow-2xl rounded-[2.5rem] relative modal-pop flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase theme-text-muted tracking-widest ml-1">Departamento Padre *</label>
                                <div className="relative">
                                    ...
                                    <select 
                                        ...
                                        required 
                                        className="w-full pl-12 pr-10 py-4 theme-surface border theme-border rounded-xl theme-text-main text-sm font-bold outline-none focus:ring-2 transition-all shadow-sm appearance-none"
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase theme-text-muted tracking-widest ml-1">Nombre del Área *</label>
                                <input 
                                    type="text" 
                                    value={data.nombre} 
                ...
                <div className="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xl animate-fade-in" onClick={() => setModalEliminar(false)}>
                    <div className="w-full max-w-sm theme-surface border-2 border-red-500/30 shadow-2xl rounded-[2.5rem] p-8 flex flex-col items-center gap-6 relative modal-pop" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="text-center space-y-2 relative z-10">
                            <h3 className="text-xl font-black uppercase italic tracking-tighter text-red-600 dark:text-red-400 m-0">¿Eliminar Área?</h3>
                            <p className="text-sm font-bold theme-text-muted leading-snug">Estás a punto de eliminar el área <span className="font-black theme-text-main">"{itemActual?.nombre}"</span>. Esta acción no se puede deshacer.</p>
                        </div>
