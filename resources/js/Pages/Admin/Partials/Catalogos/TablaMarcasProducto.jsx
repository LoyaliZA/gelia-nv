...
import { Tag, Edit2, Trash2, Plus, Save, AlertTriangle, Upload } from 'lucide-react';
...
export default function TablaMarcasProducto({ datos = [] }) {
    const [modalAbierto, setModalAbierto] = useState(false);
    const [modalEliminar, setModalEliminar] = useState(false);
    const [modalImportar, setModalImportar] = useState(false);
    const [itemActual, setItemActual] = useState(null);
    const { data, setData, post, put, processing, reset, errors } = useForm({ nombre: '', activo: true });

    const handleSubmit = (e) => {
        e.preventDefault();
        const accion = itemActual ? put : post;
        const ruta = itemActual ? route('admin.catalogos.marcas_producto.update', itemActual.id) : route('admin.catalogos.marcas_producto.store');
        accion(ruta, { onSuccess: () => { setModalAbierto(false); reset(); } });
    };

    return (
        <div>
            <GeliaLoader isVisible={processing} message="Guardando marca_" />
            <div className="p-6 border-b theme-border flex justify-between flex-wrap gap-4">
                <h2 className="text-xl font-black italic uppercase m-0 flex items-center gap-2 theme-text-main"><Tag className="w-5 h-5" /> Marcas de Producto_</h2>
                ...
                <tbody>
                    {datos.map((item) => (
                        <tr key={item.id} className="border-b theme-border">
                            ...
                            <td className="px-6 py-4 text-[10px] font-bold theme-text-main">{item.activo ? 'Activo' : 'Inactivo'}</td>
                            <td className="px-6 py-4 text-right">
                                <button onClick={() => { setItemActual(item); setData({ nombre: item.nombre, activo: item.activo }); setModalAbierto(true); }} className="p-2 theme-element border theme-border rounded-xl mr-2"><Edit2 className="w-4 h-4" /></button>
            ...
            {modalAbierto && createPortal(
                <div className={THEME_MODAL_OVERLAY} onClick={() => setModalAbierto(false)}>
                    <div className={`${THEME_MODAL_SHELL} max-w-md p-8 modal-pop`} onClick={(e) => e.stopPropagation()}>
                        <h3 className="text-xl font-black italic uppercase theme-text-main mb-6">{itemActual ? 'Editar' : 'Nueva'} Marca</h3>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <input required value={data.nombre} onChange={(e) => setData('nombre', e.target.value)} className="theme-input w-full px-4 py-3 font-bold" placeholder="Nombre marca" />
            ...
            {modalEliminar && createPortal(
                <div className={THEME_MODAL_OVERLAY} onClick={() => setModalEliminar(false)}>
                    <div className={`${THEME_MODAL_SHELL} p-8 text-center modal-pop`} onClick={(e) => e.stopPropagation()}>
                        ...
                        <button onClick={() => router.delete(route('admin.catalogos.marcas_producto.destroy', itemActual.id), { onSuccess: () => setModalEliminar(false) })} className="px-6 py-3 bg-red-600 text-white rounded-xl font-black uppercase text-[10px]">Eliminar</button>
                    </div>
                </div>, document.body
            )}
            {modalImportar && <ModalImportarCatalogo config={IMPORTACION_CATALOGOS.marcas_producto} onClose={() => setModalImportar(false)} />}
        </div>
    );
}
