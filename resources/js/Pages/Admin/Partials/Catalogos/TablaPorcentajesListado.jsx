...
import { Percent, Edit2, Trash2, Plus, X, Save } from 'lucide-react';
import GeliaLoader from '../../../../Components/GeliaLoader';

export default function TablaPorcentajesListado({ datos = [], listas = [] }) {
    ...
    const handleSubmit = (e) => {
        ...
        const ruta = itemActual
            ? route('admin.catalogos.porcentajes_listado.update', itemActual.id)
            : route('admin.catalogos.porcentajes_listado.store');
        accion(ruta, { onSuccess: () => { setModalAbierto(false); reset(); } });
    };

    const confirmDelete = () => {
        destroy(route('admin.catalogos.porcentajes_listado.destroy', itemActual.id), {
    ...
    return (
        <div>
            <GeliaLoader isVisible={processing} message="Procesando Listados_" />
            ...
            <div className="p-6 md:p-8 border-b theme-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="p-3 rounded-2xl" style={{ backgroundColor: 'color-mix(in srgb, var(--color-primario) 15%, transparent)' }}>
                        <Percent className="w-5 h-5" style={{ color: 'var(--color-primario)' }} />
                    </div>
                    <div>
                        <h2 className="text-xl font-black italic theme-text-main uppercase tracking-tighter m-0">Listados (Resurtido)_</h2>
                        <p className="text-[10px] theme-text-muted font-bold uppercase tracking-widest mt-0.5">
                            Porcentajes para generación de listas Excel · ej. BRONCE 12.39%, PLATA 14.14%
                        </p>
                    </div>
                </div>
            ...
            {modalAbierto && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-xl" onClick={() => setModalAbierto(false)}>
                    <div className="w-full max-w-lg theme-surface border shadow-2xl rounded-[2.5rem] p-8 modal-pop" onClick={e => e.stopPropagation()}>
                        <h2 className="text-xl font-black italic uppercase mb-6 flex items-center gap-3"><Percent className="w-5 h-5" style={{ color: 'var(--color-primario)' }} /> Porcentaje Listado_</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <input type="number" step="0.01" min="0" max="100" value={data.porcentaje_descuento} onChange={e => setData('porcentaje_descuento', e.target.value)} required placeholder="Ej. 14.14" className="w-full px-5 py-4 theme-surface border theme-border rounded-xl font-bold outline-none" />
            ...
            {modalEliminar && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-xl">
                    <div className="theme-surface border rounded-[2.5rem] p-8 max-w-md w-full text-center space-y-4">
                        <p className="font-black uppercase text-red-600">¿Eliminar porcentaje de listado?</p>
