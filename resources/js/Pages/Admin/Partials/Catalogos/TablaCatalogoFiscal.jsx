...
/**
 * Tabla CRUD para catálogos fiscales (codigo + nombre + activo).
 */
export default function TablaCatalogoFiscal({
    datos = [],
    titulo,
    subtitulo = 'registros',
    icon: Icon,
    ...
    const abrirNuevo = () => {
        setItemActual(null);
        reset();
        setData({ codigo: '', nombre: '', activo: true });
    ...
    return (
        <div>
            ...
            <div className="p-6 md:p-8 border-b theme-border flex items-center justify-between gap-4 flex-wrap">
                ...
                <table className="w-full border-collapse">
                    <thead>
                        <tr className="border-b-2 border-[var(--color-primario)]/30">
                            <th className="px-6 py-4 text-left text-[9px] font-black theme-text-muted uppercase tracking-widest">Código_</th>
                        ...
                        {datos.map((item) => (
                            <tr key={item.id} className="group border-b theme-border last:border-0 hover:ring-2 hover:ring-inset hover:ring-[var(--color-primario)]/30">
                                <td className="px-6 py-5">
                                    <p className="text-sm font-black theme-text-main uppercase tracking-wider m-0">{item.codigo}</p>
                                </td>
                                <td className="px-6 py-5">
                                    <p className="text-sm font-bold theme-text-main m-0">{item.nombre}</p>
                                </td>
                                <td className="px-6 py-5">
                ...
                <div className={`${THEME_MODAL_OVERLAY} z-[200]`} onClick={() => setModalAbierto(false)}>
                    <div className="w-full max-w-md theme-surface border theme-border rounded-[2rem] p-8 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div>
                                <label className="text-[10px] font-black uppercase theme-text-muted tracking-widest">Código SAT_</label>
                                <input type="text" required maxLength={10} value={data.codigo} onChange={(e) => setData('codigo', e.target.value)} className={`${THEME_INPUT} w-full mt-2 text-sm font-bold`} />
            ...
            {modalEliminar && createPortal(
                <div className={`${THEME_MODAL_OVERLAY} z-[200]`} onClick={() => setModalEliminar(false)}>
                    <div className="w-full max-w-sm theme-surface border theme-border rounded-[2rem] p-8 shadow-2xl text-center" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p className="text-sm theme-text-muted mb-6">¿Eliminar «{itemActual?.codigo} — {itemActual?.nombre}»?</p>
