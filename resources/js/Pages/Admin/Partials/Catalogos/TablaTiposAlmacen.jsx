...
import { Warehouse, Edit2, Trash2, Plus, Save, AlertTriangle, Upload } from 'lucide-react';
...
export default function TablaTiposAlmacen({ datos = [] }) {
    ...
    const handleSubmit = (e) => {
        ...
        const ruta = itemActual ? route('admin.catalogos.tipos_almacen.update', itemActual.id) : route('admin.catalogos.tipos_almacen.store');
        accion(ruta, { onSuccess: () => { setModalAbierto(false); reset(); } });
    };

    return (
        <div>
            <GeliaLoader isVisible={processing} message="Guardando tipo_" />
            <div className="p-6 md:p-8 border-b theme-border flex justify-between items-center flex-wrap gap-4">
                <h2 className="text-xl font-black italic uppercase m-0 flex items-center gap-2 theme-text-main"><Warehouse className="w-5 h-5" /> Tipos de Almacén_</h2>
                <div className="flex gap-2">
                    ...
                    <button onClick={() => { setItemActual(null); reset(); setModalAbierto(true); }} className="px-6 py-3 rounded-2xl font-black uppercase text-xs text-white" style={{ backgroundColor: 'var(--color-primario)' }}><Plus className="w-4 h-4 inline" /> Nuevo</button>
                </div>
            </div>
            ...
            {modalAbierto && createPortal(
                <div className={THEME_MODAL_OVERLAY} onClick={() => setModalAbierto(false)}>
                    <div className={`${THEME_MODAL_SHELL} max-w-md p-8 modal-pop`} onClick={(e) => e.stopPropagation()}>
                        <h3 className="text-xl font-black italic uppercase theme-text-main mb-6">{itemActual ? 'Editar' : 'Nuevo'} Tipo</h3>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <button type="submit" className="w-full py-3 text-white rounded-xl font-black uppercase" style={{ backgroundColor: 'var(--color-primario)' }}>Guardar</button>
                        </form>
                    </div>
                </div>, document.body
            )}
            {modalEliminar && createPortal(
                <div className={THEME_MODAL_OVERLAY} onClick={() => setModalEliminar(false)}>
                    <div className={`${THEME_MODAL_SHELL} p-8 rounded-[2rem] text-center modal-pop`} onClick={(e) => e.stopPropagation()}>
                        <AlertTriangle className="w-10 h-10 text-red-500 mx-auto mb-4" />
                        <p className="theme-text-main mb-4">¿Eliminar «{itemActual?.nombre}»?</p>
                        <button onClick={() => router.delete(route('admin.catalogos.tipos_almacen.destroy', itemActual.id), { onSuccess: () => setModalEliminar(false) })} className="px-6 py-3 bg-red-600 text-white rounded-xl font-black text-[10px] uppercase">Eliminar</button>
                    </div>
                </div>, document.body
            )}
            {modalImportar && <ModalImportarCatalogo config={IMPORTACION_CATALOGOS.tipos_almacen} onClose={() => setModalImportar(false)} />}
        </div>
    );
}
