...
export default function TablaListas({ datos = [] }) {
    ...
    const handleSubmit = (e) => {
        ...
        const ruta = itemActual ? route('admin.catalogos.listas.update', itemActual.id) : route('admin.catalogos.listas.store');
        accion(ruta, { onSuccess: () => { setModalAbierto(false); reset(); } });
    };

    const confirmDelete = () => {
        formEliminar.post(route('admin.catalogos.listas.destroy', itemActual.id), {
            _method: 'delete',
            onSuccess: () => { setModalEliminar(false); setItemActual(null); formEliminar.reset(); }
        });
    };

    return (
        <div>
            <GeliaLoader isVisible={processing || formEliminar.processing} message="Procesando Listas_" />
            ...
            <div className="p-6 md:p-8 border-b theme-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black italic theme-text-main uppercase tracking-tighter m-0">Listas de Precios_</h2>
                    ...
                    <div className="w-full max-w-lg theme-surface theme-border border shadow-2xl rounded-[2.5rem] relative modal-pop flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                        <div className="p-6 md:p-8 border-b theme-border flex items-center justify-between shrink-0">
                            <h2 className="text-xl font-black italic uppercase tracking-tighter theme-text-main flex items-center gap-3 m-0 leading-none">
                                ...
                                {itemActual ? 'Editar' : 'Nueva'} Lista_
                            </h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase theme-text-muted tracking-widest ml-1">Nombre de la Lista_</label>
                                <input 
                                    type="text" 
                                    value={data.nombre} 
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase theme-text-muted tracking-widest ml-1">Monto Base Requerido_</label>
                                <input 
                                    type="number" 
                                    step="0.01" 
            ...
            {/* Modal Eliminar Especial (Revinculación) */}
            {modalEliminar && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-xl animate-fade-in" onClick={() => setModalEliminar(false)}>
                    <div className="w-full max-w-md theme-surface border-2 border-red-500/30 shadow-2xl rounded-[2.5rem] p-8 flex flex-col items-center gap-6 relative modal-pop" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="text-center space-y-2 relative z-10">
                            <h3 className="text-xl font-black uppercase italic tracking-tighter text-red-600 dark:text-red-400 m-0">¿Eliminar Lista?</h3>
                            <p className="text-sm font-bold theme-text-muted leading-snug">Los clientes asignados a <span className="font-black theme-text-main">"{itemActual?.nombre}"</span> quedarán huérfanos. Selecciona una lista destino para reubicarlos.</p>
                        </div>
                        
                        <div className="w-full space-y-2 relative z-10">
                            <label className="text-[10px] font-black uppercase theme-text-muted tracking-widest ml-1">Mover clientes a_</label>
                            <select 
                                value={formEliminar.data.reubicar_en_id} 
                                onChange={e => formEliminar.setData('reubicar_en_id', e.target.value)}
                                className="w-full px-5 py-4 theme-surface border theme-border rounded-xl theme-text-main text-sm font-bold outline-none focus:ring-2 transition-all shadow-sm appearance-none"
                                style={{ '--tw-ring-color': 'var(--color-primario)' }}
                            >
                                <option value="">-- No reubicar (Dejar huérfanos) --</option>
                                {datos.filter(d => d.id !== itemActual?.id).map(lista => (
                        ...
                        <div className="flex w-full gap-3 mt-2 relative z-10">
                            <button onClick={() => setModalEliminar(false)} className="flex-1 py-3 px-4 theme-element border theme-border rounded-2xl text-xs font-bold theme-text-main transition-transform hover:scale-105 shadow-sm outline-none">Cancelar</button>
                            <button onClick={confirmDelete} disabled={formEliminar.processing} className="flex-1 py-3 px-4 bg-red-500 hover:bg-red-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all hover:scale-105 shadow-md flex items-center justify-center gap-2 outline-none disabled:opacity-50"><Trash2 className="w-4 h-4" /> Eliminar</button>
                        </div>
                    </div>
                </div>, document.body
            )}
