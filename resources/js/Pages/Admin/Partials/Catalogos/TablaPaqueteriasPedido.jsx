...
export default function TablaPaqueteriasPedido({ datos = [] }) {
    ...
    const abrirEditar = (item) => {
        ...
        setData({
            nombre: item.nombre,
            costo_seguro_default: item.costo_seguro_default ?? 0,
            activo: item.activo,
        });
        setModalAbierto(true);
    };
                ...
                <table className="w-full border-collapse">
                    <thead>
                        <tr className="border-b-2 border-[var(--color-primario)]/30">
                            ...
                            <th className="px-6 py-4 text-left text-[9px] font-black theme-text-muted uppercase tracking-widest">Seguro default_</th>
                    ...
                    <tbody>
                        {datos.map((item) => (
                            <tr key={item.id} className="border-b theme-border last:border-0">
                                ...
                                <td className="px-6 py-5 text-sm font-bold theme-text-muted">${Number(item.costo_seguro_default || 0).toFixed(2)}</td>
                ...
                <div className={`${THEME_MODAL_OVERLAY} z-[200]`} onClick={() => setModalAbierto(false)}>
                    <div className="w-full max-w-md theme-surface border theme-border rounded-[2rem] p-8 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div>
                                <label className="text-[10px] font-black uppercase theme-text-muted tracking-widest">Costo seguro default_</label>
                                <input type="number" step="0.01" min="0" value={data.costo_seguro_default} onChange={(e) => setData('costo_seguro_default', e.target.value)} className={`${THEME_INPUT} w-full mt-2 text-sm font-bold`} />
                            </div>
