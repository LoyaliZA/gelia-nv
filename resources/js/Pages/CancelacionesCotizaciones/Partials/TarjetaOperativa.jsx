        ...
        >
            <div className="flex flex-wrap items-start justify-between gap-3 mb-4 min-w-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                        type="button"
                        onClick={(e) => onMenu(e, solicitud)}
                        className="p-2 rounded-xl theme-element border theme-border outline-none hover:border-[var(--color-primario)] transition-colors"
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                ...
                {solicitud.numero_pedido && (
                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black uppercase theme-element border theme-border">
                        <Hash className="w-3 h-3 shrink-0" style={{ color: 'var(--color-primario)' }} /> Ped. {solicitud.numero_pedido}
                    </span>
                )}
                ...
                {solicitud.cancelacion_solicitada_at && estadoNombre !== 'Cancelada' && (
                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black uppercase bg-red-500/10 text-red-600 border border-red-500/30">
                        <Ban className="w-3 h-3 shrink-0" /> Cancelación solicitada
                    </span>
                )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] font-bold theme-text-muted mb-4 min-w-0">
                <span className="inline-flex items-center gap-1 min-w-0">
                    ...
                    <span className="truncate">{solicitud.vendedor?.name}</span>
                </span>
            ...
            <div className="flex flex-wrap gap-2 pt-4 mt-auto border-t theme-border">
                {estadoId === 1 && puedeResponder && (
                    <>
                        ...
                        >
                            <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> Aprobar
                        </button>
                        <button
                            type="button"
                ...
                {estadoId === 2 && puedeVerificar && (
                    ...
                    >
                        <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> Verificar
                    </button>
                )}
            </div>
        </article>
