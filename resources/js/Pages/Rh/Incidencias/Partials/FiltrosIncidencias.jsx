        ...
        <div className="p-4 md:p-6 border-b theme-border space-y-4">
            <div className="flex flex-wrap gap-2">
                {TABS_ESTADO.map((tab) => (
                    ...
                    >
                        {tab === 'TODAS' ? 'Todas' : tab === 'APLICADAS' ? 'Aplicadas' : tab.charAt(0) + tab.slice(1).toLowerCase()}
                    </button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <select value={filtros.catalogo_tipo_falta_id || ''} onChange={(e) => aplicar({ catalogo_tipo_falta_id: e.target.value || undefined })} className={`px-4 py-3 rounded-2xl ${THEME_SELECT} text-[11px] font-bold`}>
