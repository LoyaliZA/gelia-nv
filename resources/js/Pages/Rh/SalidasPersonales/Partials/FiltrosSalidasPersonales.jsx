    ...
    return (
        <div className="p-4 md:p-6 border-b theme-border space-y-4">
            ...
            <div className="flex flex-col lg:flex-row gap-3">
                ...
                />
                <button type="button" onClick={ejecutarBusqueda} className="px-5 py-3 rounded-2xl text-[10px] font-black uppercase text-white flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--color-primario)' }}>
                    <Filter className="w-4 h-4 mr-2" /> Buscar
                </button>
                <button type="button" onClick={() => aplicar({ fecha_inicio: new Date().toISOString().split('T')[0], fecha_fin: new Date().toISOString().split('T')[0] })} className="px-5 py-3 rounded-2xl text-[10px] font-black uppercase theme-element theme-border border flex items-center justify-center shrink-0">
                    <Calendar className="w-4 h-4 mr-2" /> Hoy
                </button>
                {hayFiltros && (
                    <button type="button" onClick={limpiar} className="px-5 py-3 rounded-2xl text-[10px] font-black uppercase theme-element theme-border border flex items-center justify-center shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <input type="date" value={filtros.fecha_inicio || ''} onChange={(e) => aplicar({ fecha_inicio: e.target.value || undefined })} className={`${THEME_INPUT} px-4 py-3 rounded-2xl text-[11px] font-bold`} />
                <input type="date" value={filtros.fecha_fin || ''} onChange={(e) => aplicar({ fecha_fin: e.target.value || undefined })} className={`${THEME_INPUT} px-4 py-3 rounded-2xl text-[11px] font-bold`} />
            </div>
        </div>
    );
}
