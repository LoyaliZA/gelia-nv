...
import { BITACORA_TONO, BTN_CERRAR_MODAL, TEXTO_AVISO } from './traspasosStyles';
...
const BotonEvidencia = ({ url, label = 'Ver evidencia', onAbrir, miniatura = false }) => {
    ...
    return (
        <button
            type="button"
            onClick={() => onAbrir(url)}
            className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest theme-text-info hover:opacity-80 transition-opacity w-fit outline-none"
        >
...
const estilosPaso = {
    ok: { ...BITACORA_TONO.ok, Icon: CheckCircle2 },
    error: { ...BITACORA_TONO.error, Icon: AlertOctagon },
    info: { iconWrap: 'gelia-icon-box theme-text-primario', rail: 'theme-border', label: 'theme-text-primario', Icon: ShieldCheck },
};
                    ...
                    >
                        <div className="p-6 md:p-8 flex justify-between items-start gap-3 border-b theme-border shrink-0">
                            <div className="flex items-center gap-4 min-w-0">
                                <History className="w-9 h-9 shrink-0 theme-text-primario" />
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 flex-1 overflow-hidden p-6 md:p-8">
                            <div className="theme-element border theme-border rounded-3xl p-6 md:p-8 overflow-y-auto custom-scrollbar">
                                <h3 className="text-sm font-black uppercase tracking-widest mb-6 m-0 theme-text-primario">
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="p-4 rounded-xl gelia-panel-suave">
                                            ...
                                            <div className="space-y-2">
                                                {productos.map((p) => {
                                                    ...
                                                    <div key={p.id} className="flex flex-col gap-1 text-xs border theme-border rounded-xl px-3 py-2">
                                                        ...
                                                        {incidencias.map((rev) => (
                                                            <span key={rev.id} className={TEXTO_AVISO}>
                                    ...
                                    {auditorias.map((registro) => {
                                        ...
                                        return (
                                            <div key={registro.id} className="relative flex flex-col ml-10">
                                                <div className={`absolute -left-[3.5rem] top-1 w-10 h-10 rounded-full border-4 theme-surface flex items-center justify-center shadow-md z-10 ${estilo.iconWrap}`}>
                                                    <IconoPaso className="w-4 h-4" />
                                                </div>

                                                <div className={`theme-element border ${estilo.rail} p-5 md:p-6 rounded-3xl shadow-sm`}>
