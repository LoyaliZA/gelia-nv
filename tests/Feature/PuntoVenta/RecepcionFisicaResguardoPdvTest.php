...
class RecepcionFisicaResguardoPdvTest extends TestCase
{
    ...
    public function test_confirmacion_gerente_crea_bultos_y_transiciona_a_recibido(): void
    {
        ...
        $response = $this->actingAs($this->usuario)->putJson(
            ...
            $this->payloadRecepcion($resguardo, $clave)
        );
    ...
    public function test_validaciones_obligatorias_y_permiso(): void
    {
        ...
        $this->actingAs($this->usuario)->putJson(
            ...
            ->assertJsonValidationErrors(['version', 'idempotency_key']);
    ...
    public function test_segunda_confirmacion_rechazada(): void
    {
        Event::fake([RecepcionFisicaPdvCompletada::class]);
    ...
    public function test_transaccion_fallida_no_deja_bultos_ni_eventos(): void
    {
        Event::fake([RecepcionFisicaPdvCompletada::class]);

        $resguardo = $this->crearResguardoPendiente();

        $this->mock(SincronizarEstatusSucursalPedidoBmaService::class, function ($mock) {
            $mock->shouldReceive('desdeResguardo')
                ->once()
                ->andThrow(new \RuntimeException('fallo forzado sincronizacion'));
        });
        ...
        try {
            ...
            $this->assertSame('fallo forzado sincronizacion', $e->getMessage());
        }

        $this->assertSame(0, ResguardoPdvBulto::query()->count());
        $this->assertSame(0, ResguardoPdvEvento::query()->count());
        $this->assertSame(ResguardoPdv::ESTADO_PENDIENTE_RECEPCION, $resguardo->fresh()->estado);
        Event::assertDispatchedTimes(RecepcionFisicaPdvCompletada::class, 0);
    }
